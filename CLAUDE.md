...
- **manual-work.txt**: 本番環境での確認・操作が必要で今すぐ完了できない「保留中タスク」を集約
  (本番デプロイ、本番データ操作、拡張の実機テスト等)。以下を徹底する:
  - コード実装が済んでも本番反映・本番確認が必要なものは必ずここに記録する
  - 新たに同種のタスクが出たら都度追記する(M-1, M-2… と採番)
  - 本番確認ができるようになった際に、このファイルを見れば残タスクが分かる状態を保つ
